...
class ForecastingEncoderDecoder(nn.Module):
    def __init__(self, cfg):
        ...
        self.feature_fusion_egovlp = nn.Linear( 3*1024 ,cfg.MODEL.MULTI_INPUT_FEATURES)
        #EgoVLP: 3*1024, MViT & Slowfast & Stillfast: 2*1024
    ...
    def forward(self, x, tgts=None):
        ...
        #print(f'length of features: {len(features)}, {features[0].shape}')
        if self.feature_fusion_egovlp !=None :
            features = [self.feature_fusion_egovlp(feature) for feature in features]
