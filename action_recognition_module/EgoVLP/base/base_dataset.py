...
def sample_frames_clips_random(start, end, vlen, acc_samples):
    start = max(0, start)
    end = min(vlen, end)
...
def read_frames_decord_start_end(video_path, start, end, num_frames, sample='rand'):
    video_reader = decord.VideoReader(video_path, num_threads=1)
    vlen = len(video_reader)
    if sample=='rand':
        frame_idxs = sample_frames_clips_random(start, end, vlen, num_frames + 1)
    else: # uniform
